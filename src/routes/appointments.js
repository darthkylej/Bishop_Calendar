...
function nextCoveredDue(due,appointmentDate,count,unit){
  ...
  do { next=advanceDate(next,count,unit); guard++; } while(next<=earlyLimit&&guard<500);
  return next;
}
async function coverRecurring(sql,id,appointmentDate){
  ...
  if(!row||row.one_time) return;
  const due=ymdValue(row.next_due_date); if(!due)throw new Error('Recurring interview has an invalid due date.');
  const next=nextCoveredDue(due,appointmentDate,Number(row.frequency_count),row.frequency_unit);
  if(next!==due) await sql`UPDATE recurring_interviews SET next_due_date=${next},updated_at=now() WHERE id=${id}`;
}
async function restoreRecurringAfterCancellation(sql,appointment,tz){
  if(!appointment?.recurring_interview_id) return;
  await ensureNeedSchema(sql); await ensureAppointmentSchema(sql);
  const row=(await sql`SELECT id,frequency_count,frequency_unit,next_due_date,one_time FROM recurring_interviews WHERE id=${appointment.recurring_interview_id}`)[0];
  if(!row) return;
  if(row.one_time){ await sql`UPDATE recurring_interviews SET active=true,updated_at=now() WHERE id=${row.id}`; return; }
  const prior=ymdValue(appointment.recurring_due_date); if(!prior) return;
  const appointmentDate=ymd(new Date(appointment.start_at),tz);
  const coveredDue=nextCoveredDue(prior,appointmentDate,Number(row.frequency_count),row.frequency_unit);
  const current=ymdValue(row.next_due_date);
  if(current===coveredDue) await sql`UPDATE recurring_interviews SET next_due_date=${prior},active=true,updated_at=now() WHERE id=${row.id}`;
}
export async function week(request,env,user){
  ...
  const sql=db(env); await ensureAppointmentSchema(sql); const settings=await sql`SELECT key,value FROM settings`; const sm=Object.fromEntries(settings.map(x=>[x.key,x.value]));
...
export async function create(request,env,user){
  if(!canSchedule(user))return error('Forbidden.',403); const d=await body(request); if(!d)return error('Invalid request.'); const sql=db(env); await ensureAppointmentSchema(sql); await ensureNeedSchema(sql);
  ...
  const personName=String(d.person_name||'').trim();
  let recurringId=d.recurring_interview_id?Number(d.recurring_interview_id):null, recurringDue=null;
  if(recurringId){
    const rr=(await sql`SELECT id,next_due_date,assigned_to_counselor FROM recurring_interviews WHERE id=${recurringId} AND active=true`)[0];
    ...
    recurringDue=ymdValue(rr.next_due_date)||null;
  } else if(personName){
    const matches=await sql`SELECT id,next_due_date FROM recurring_interviews
      WHERE active=true AND assigned_to_counselor IS NULL AND person_name=${personName}
      ORDER BY next_due_date,id LIMIT 2`;
    if(matches.length===1){
      recurringId=Number(matches[0].id);
      recurringDue=ymdValue(matches[0].next_due_date)||null;
    }
  }
  try{
    const r=await sql`INSERT INTO appointments(start_at,end_at,person_name,appointment_type,notes,confirmation_status,recurring_interview_id,recurring_due_date,created_by,updated_by) VALUES(${d.start_at},${d.end_at},${personName},${d.appointment_type||'Interview'},${d.notes||null},${confirmation(d.confirmation_status)},${recurringId},${recurringDue},${user.id},${user.id}) RETURNING *`;
...
export async function update(request,env,user,id){
  if(!canSchedule(user))return error('Forbidden.',403); const d=await body(request); if(!d)return error('Invalid request.'); const sql=db(env); await ensureAppointmentSchema(sql);
  const old=(await sql`SELECT * FROM appointments WHERE id=${id}`)[0]; if(!old)return error('Appointment not found.',404);
  const start=d.start_at||old.start_at,end=d.end_at||old.end_at; const settings=await sql`SELECT key,value FROM settings`; const sm=Object.fromEntries(settings.map(x=>[x.key,x.value])); const tz=sm.timezone||'America/Chicago';
  if(!isBishop(user)&&!await isAvailable(sql,start,end,tz))return error('That time is outside the bishop’s available hours.',409);
  const newStatus=d.status??old.status;
  try{
    const r=await sql`UPDATE appointments SET start_at=${start},end_at=${end},person_name=${d.person_name??old.person_name},appointment_type=${d.appointment_type??old.appointment_type},notes=${d.notes??old.notes},confirmation_status=${confirmation(d.confirmation_status,old.confirmation_status||'confirmed')},status=${newStatus},updated_by=${user.id},updated_at=now() WHERE id=${id} RETURNING *`;
    if(old.status!=='cancelled'&&newStatus==='cancelled') await restoreRecurringAfterCancellation(sql,{...old,start_at:start},tz);
    else if(old.status==='cancelled'&&newStatus!=='cancelled'&&old.recurring_interview_id) await coverRecurring(sql,old.recurring_interview_id,ymd(new Date(start),tz));
    return json({appointment:r[0]});
  }catch(e){ if(String(e).includes('appointments_no_overlap')||String(e).includes('conflict'))return error('That time overlaps an existing appointment.',409); throw e; }
}
export async function remove(request,env,user,id){
  if(!canSchedule(user))return error('Forbidden.',403); const sql=db(env); await ensureAppointmentSchema(sql);
  const old=(await sql`SELECT * FROM appointments WHERE id=${id}`)[0]; if(!old)return error('Appointment not found.',404);
  const r=await sql`UPDATE appointments SET status='cancelled',updated_by=${user.id},updated_at=now() WHERE id=${id} RETURNING id`;
  if(old.status!=='cancelled'){
    const settings=await sql`SELECT key,value FROM settings`; const sm=Object.fromEntries(settings.map(x=>[x.key,x.value]));
    await restoreRecurringAfterCancellation(sql,old,sm.timezone||'America/Chicago');
  }
  return json({ok:true});
}
